...
MyVideoHandler::MyVideoHandler(int device, bool saveKeyFrame, bool saveVideo)
	: mCapture(device)
...
MyVideoHandler::MyVideoHandler(const string& file, bool saveKeyFrame)
	: mCapture(file)
	, mSaveKeyFrame(saveKeyFrame)
	, mFromCam(false)
	, mVideoFPS(0)
